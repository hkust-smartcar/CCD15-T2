...
uint16_t App::Get_mid(uint16_t* m_ccd_data, uint16_t avg, int ccdNumber, uint16_t* midData, CCD_COLOR* color, uint16_t* regionTotalNumber, uint16_t* nowMid){
	...
	int prev_total_white = 0;
	if(ccdNumber == 1){
		prev_total_white = m_total_white_1;
		m_total_white_1 = total_white;
	}else if(ccdNumber == 2){
		prev_total_white = m_total_white_2;
...
//				if(i > 0 && color[midData[i]] == CCD_WHITE && ((int)regionEdge[i+1] - (int)regionEdge[i])>((int)regionEdge[closestMid+1] - (int)regionEdge[closestMid]) ){
////					m_car.m_buzzer.SetBeep(true);
//					return midData[i];
//				}
//			}
...
void App::PitBalance(Pit*){
	static bool canTurnOff = false;
	static uint32_t prevcount = 0;
	if(m_car.m_infrared.GetResultF() >= 1.0f){
		m_car.m_infrared_switch.Clear();
		prevcount = m_pit_count;
	}

	if(m_pit_count - prevcount > 100){
...
//		if(m_turn_error_1 < 0 && m_turn_error_2 > 0 || m_turn_error_1 > 0 && m_turn_error_2 < 0){
//			m_turn_kp = m_original_turn_kp * 0.1f;
//			m_turn_kd = m_original_turn_kd * 2.5f;
//		}else{
...
//		}
		 ...
		 * 90 degree detection
		 */
		m_width1 = ((int)m_edge_data_1[m_cur_mid_i_1+1] - (int)m_edge_data_1[m_cur_mid_i_1]);
		m_width2 = ((int)m_edge_data_2[m_cur_mid_i_2+1] - (int)m_edge_data_2[m_cur_mid_i_2]);
...
//			if(m_turn_error > 10/*abs(m_turn_error - m_turn_prev_error)>15*/){
//				m_turn_kd = m_original_turn_kd * 0.5f;
//			}else{
//				m_turn_kd = m_original_turn_kd;
//			}
			if(abs(m_turn_error) > 5){
//				if(m_speedInMetrePerSecond > 1.85f){
//					m_turn_kp = 4.0f;
//					m_turn_kd = 0.9f;
//				}
//				if(m_speedInMetrePerSecond > 1.7f){
//					m_turn_kp = 5.0f;
//					m_turn_kd = 0.8f;
//				}
			}else{
...
//			m_turn_powerr = libutil::Clamp<int16_t>(-190,m_movavgturn.GetAverage(),190);
//			m_turn_powerl = libutil::Clamp<int16_t>(-190,-m_movavgturn.GetAverage(),190);
			m_turn_powerr = m_movavgturn.GetAverage();
		...
		if(m_car.m_lcdupdate){
			...
			rect_.h = 16;
			m_car.m_lcd.SetRegion(rect_);
//			m_lcd_typewriter.WriteString(String::Format("IR %.1f\n", m_car.m_infrared.GetResultF()).c_str());
			m_lcd_typewriter.WriteString(String::Format("%d\n",m_car.m_ultrasonic.GetDistance()).c_str());
...
//					if(m_speed_error < 0.0f){
//						m_speed_output = 200.0f * m_speed_error + 0.001f * m_total_speed;
//						m_balcon[6] = -(2.0f * m_speed_error + 0.0f * (m_speed_error-m_prev_speed)/0.02f + 0.0f * m_total_speed);
//					}else if(m_speed_error > 0.2f){
//						m_speed_output = 200.0f * m_speed_error + 0.001f * m_total_speed;
//						m_balcon[6] = -(2.0f * m_speed_error + 0.0f * (m_speed_error-m_prev_speed)/0.02f + 0.0f * m_total_speed);
//					}/*else{
//						m_speed_output = 100.0f * m_speed_error + 0.8f * m_total_speed;
//						m_balcon[6] = -(9.0f * m_speed_error + 0.0f * (m_speed_error-m_prev_speed)/0.02f + 0.03f * m_total_speed);
//					}*/
					if(m_speed_error > 0.4f){
						m_speed_output = (int16_t)(500.0f * m_speed_error + 2.16f * m_total_speed);
						m_balcon[6] = -(4.0f * m_speed_error + 0.0f * (m_speed_error-m_prev_speed)/0.02f + 0.01f * m_total_speed);
					}else{
						m_speed_output = (int16_t)(500.0f * m_speed_error + 2.16f * m_total_speed);
						m_balcon[6] = -(4.0f * m_speed_error + 0.0f * (m_speed_error-m_prev_speed)/0.02f + 0.01f * m_total_speed);
					}

					if(m_car.m_ultrasonic.GetDistance() < 300){
//						m_car.m_buzzer.SetBeep(true);
//						m_speed_output += 850;
//						m_hold_count2 = 10;
					}
...
//					if(m_turn_error_2 < 20){
//						m_speed_output = 100.0f * m_speed_error + 0.3f * m_total_speed;
//						m_balcon[6] = -(5.0f * m_speed_error + 0.0f * (m_speed_error-m_prev_speed)/0.02f + 0.03f * m_total_speed);
//
//					}
					m_prev_speed = m_speed_setpoint-m_speedInMetrePerSecond;
					m_balcon[6] = libutil::Clamp<float>(-13.0f,m_balcon[6],13.0f);
//					if(m_turn_error_2 < 3){
//						m_balcon[6] += 0.18f * abs(m_turn_error_2);
//					}else
//					if(m_turn_error_2 > 40){
//						m_balcon[6] -= 0.3f * abs(m_turn_error_2);
//					}
//			}
...
//					m_car.m_com->SendBuffer(buf, sizeof(buf));
	...
	if(m_prev_voltage <= 0.5f && m_voltage >= 1.5){
//		m_car.m_buzzer.SetBeep(true);
//		m_car.m_car_move_forward = true;
//		m_hold_count = 100;
	}
...
App::App():
	...
	m_hold_turn_kp(0.0f),
	m_hold_turn_kd(0.0f),
	m_original_turn_kp(6.0f),
	m_original_turn_kd(0.5f),
	m_turn_kp(m_original_turn_kp),
	m_turn_kd(m_original_turn_kd),
	m_found_middle_line(false),
